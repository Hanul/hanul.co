	...
	init : () => {
		...
		HanulCo.ROOM('iconLaunchRoom', (clientInfo, on, off) => {
			...
			on('launchImage', (fileId, ret) => {
				...
				IMAGEMAGICK_IDENTIFY(filePath, (features) => {
					...
					if (features.width > features.height) {
						...
						EACH(HanulCo.LaunchImageMap, (size, name) => {
							...
							IMAGEMAGICK_CONVERT([filePath, '-resize', size.height + 'x' + size.width + '^', '-gravity', 'center', '-crop', size.height + 'x' + size.width + '+0+0', filePath + '-' + name + '.png'], {
								error : (errorMsg) => {
									console.log('오류가 발생했습니다. 오류 메시지: ' + errorMsg);
								},
								success : () => {
